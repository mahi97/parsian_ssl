...
void COurBallPlacement::execute_x(){
    ...
    Vector2D behindBall = ballpos - Vector2D(pos - ballpos).normalizedVector() * 0.20;
    Circle2D validcir{pos , 0.2};
    Circle2D invalidcir{pos, 1 - 0.1};
    Vector2D sol1, sol2;
    drawer->draw(Segment2D(ballpos , ballpos + wm->ball->vel.normalizedVector()) , QColor(Qt::blue));
    ...
    if(state == BallPlacement :: PASS &&
    (invalidcir.contains(ballpos) || invalidcir.intersection(Segment2D(ballpos, ballpos + wm->ball->vel.normalizedVector()),&sol1,&sol2) > 0)){
    ...
    " -- " << validcir.intersection(Segment2D(ballpos, ballpos + wm->ball->vel.normalizedVector()),&sol1,&sol2) << " -- " << agents[minIndexPos]->pos().dist(ballpos));
    ...
    (validcir.contains(ballpos) || validcir.intersection(Segment2D(ballpos, ballpos + wm->ball->vel.normalizedVector()),&sol1,&sol2) > 0)){
    ...
    validcir.intersection(Segment2D(ballpos, ballpos + wm->ball->vel.normalizedVector()),&sol1,&sol2) == 0){
        state = BallPlacement :: GO_FOR_VALID_PASS;
    }
    if(state == BallPlacement :: RECIVE_AND_POS && !invalidcir.contains(ballpos) && !validcir.contains(ballpos)){
        state = BallPlacement :: GO_FOR_BALL;
