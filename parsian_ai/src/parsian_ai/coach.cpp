...
QList<int> CCoach::findBestPoses(int numberOfPositionAgents)
{
    ...
    OurAgents = wm->our.data->activeAgents; // TODO : fix this line
    if( goalieAgent != nullptr ){
        OurAgents.removeOne(goalieAgent->id());
    }
    for( int i=0 ; i<defenseAgents.size() ; i++ ) {
        if( OurAgents.contains(defenseAgents.at(i)->id()) ) {
            OurAgents.removeOne(defenseAgents.at(i)->id());
        }
    }
...
void CCoach::choosePlaymakeAndSupporter(bool defenseFirst)
{
    ...
    else
    {
        ...
        for(int i = 0; i < ourPlayers.size(); i++)
            nearest[ourPlayers[i]] = CSkillKick::kickTimeEstimation(agents[ourPlayers[i]], wm->field->oppGoal());
...
void CCoach::ShufflePlanIndexing(QList<SPlan*> Plans){
    ...
    for(int i=0; i<Plans.size(); i++){
        ...
        debugger->debug(QString("plan%1 cahnce : %2").arg(i).arg(Plans.at(i)->common.chance) , D_FATEME);
...
int CCoach::LFUPlan(QList<SPlan*> validPlans){
    ...
    return static_cast<int>(LFUPlanID);
}
...
//    debugger->debug(QString("chosen plan : %1").arg(staticPlayoffPlansShuffleIndexing.at(shuffleCounter)) , D_FATEME);
    PDEBUG("choose plan :", staticPlayoffPlansShuffleIndexing.at(shuffleCounter), D_FATEME);
    shuffleCounter++;
    ...
    else
    {
        if(prevValidPlans.size() > 0){
                    ...
                    if(!prevValidPlans.contains(p)){    // set the new plan's repeat
                        p->common.planRepeat = std::max(std::max(0, minChanceRepeat-1), p->common.planRepeat);
                    }
                }
        }
    }
...
void CCoach::initStaticPlay(const POMODE _mode, const QList<int>& _ourplayers) {
    ...
    NGameOff::SPlan* thePlan = nullptr;
    ...
    if (validPlans.isEmpty()) {
        DBUG("[coach] WE DONT HAVE PLAN AT ALL", D_MAHI);
        return;
    }
    ...
    lastPlan = thePlan;
    PDEBUG("chosen plan is : ",lastPlan->gui.index[3], D_MAHI);
}
...
void CCoach::LFUInit(QList<NGameOff::SPlan*> allPlans){
    ...
    DBUG("done with LFUinit" , D_FATEME);
}
...
QList<SPlan *> CCoach::getValidPlans(const POMODE _mode, const QList<int>& _ourPlayers) {
    ...
    debugger->debug(QString("playoff -> there's %1 valid Plan").arg(validPlans.size()), D_DEBUG);
    if (validPlans.isEmpty()) {
...
void CCoach::decideNull(QList<int> &_ourPlayers) {
    ...
    debugger->debug(QString("Unexpected Game State: %1 %2").arg(knowledge->stateToString(knowledge->getGameState())).arg(knowledge->getGameState()) , D_ERROR , "red");
}
