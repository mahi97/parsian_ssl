...
Matrix::Matrix(char* const init_string) {
...
Matrix::Matrix(int identity_size) {
    ...
    for (int i = 0; i < identity_size; i++) {
        for (int j = 0; j < identity_size; j++) {
            this->e(i, j) = static_cast<double>(i == j);
        }
    }
}
...
Matrix& Matrix::operator= (char* const init_string) {
    if (mat != nullptr) free(mat);
    str_init(init_string);
...
void Matrix::str_init(char* const init_string) {
    auto * str1 = (char*)malloc(strlen(init_string) + 1);
    int tcount;
    const char *delim = " ;[]";
    ...
    while (strtok(nullptr, delim) != nullptr) tcount ++;
    ...
    if (strtok(str1, ";") != nullptr) r_++;
    while (strtok(nullptr, ";") != nullptr) r_ ++;
    ...
    for (int i = 1; i < tcount; i++) {
        mat[i] = atof(strtok(nullptr, delim));
    }
    free(str1);
}
