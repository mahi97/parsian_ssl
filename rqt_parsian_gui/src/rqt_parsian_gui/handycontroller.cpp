...
void HandyController::initPlugin(qt_gui_cpp::PluginContext& context) {
    ...
    //initial value
    agentID = mUI.Agent_combobox->currentText().toInt();
    cmd->robot_id = agentID;
    speed = mUI.Speed_slide->sliderPosition();                  //no msg for it should effect in wheelspwwds
    kickspeed = mUI.Kickspeed_slide->sliderPosition();
    cmd->kickSpeed = 0;//kickspeed;
    chipkickspeed = mUI.ChipKickSpeed_slide->sliderPosition();
    cmd->kickspeedz = 0;//chipkickspeed;
    rollerspeed = mUI.RollerSpeed_slide->sliderPosition();
    cmd->roller_speed = 0;
...
void HandyController::setRobotVel(double _vtan , double _vnorm , double _w) {
    ...
    cmd->wheel1 = static_cast<float>(wheel1);
    cmd->wheel2 = static_cast<float>(wheel2);
    cmd->wheel3 = static_cast<float>(wheel3);
    cmd->wheel4 = static_cast<float>(wheel4);
}
